...
### Fixed
- **Host-page console noise can no longer leak into the container terminal — even in minified consumer builds**: the Layer C global-console capture (`src/shims/child_process.ts`) previously decided "is this the host page talking?" by matching literal package-name strings (`floating-vue`, etc.) in stack frames. That worked in dev, where stacks contain unminified `node_modules/floating-vue/...` paths, but a production consumer build bundles everything into hashed chunks (`assets/index-BQxUwFnS.js`) — no recognizable name survives, so host-page logs (e.g. `[floating-vue] patched version mounted…`) were forwarded into the process output stream and showed up in the terminal panel. The discriminator is now an allowlist keyed on a platform-owned marker instead of a name blocklist: every module the runtime evaluates carries a `//# sourceURL=almostnode:<path>` tag, and only calls whose stack contains at least one tagged frame are forwarded. Host-page calls pass through to the original console chain untouched (they are also no longer swallowed from the host's own devtools while a command runs).
- **Silent one-shot scripts now exit under streaming spawns**: a script that finished executing synchronously, produced no output and forked no children used to hang forever when spawned through `container.spawn()`/`pnpm run` (the abort-signal "long-running" mode skipped both idle and hard timeouts — real Node exits when the event loop drains). The node command now applies a 10s silent-startup grace: if nothing printed and no children are running by then, the command exits cleanly. Dev servers/watchers are unaffected (they print banners and/or fork workers). Also hardened the unhandled-rejection listener registration for embedding environments without a global event target (`globalThis.addEventListener` guard).
...
### Added
- **Source tagging for evaluated code** (`src/source-tag.ts`): `tagSource(code, path)` appends an idempotent `//# sourceURL=almostnode:<path>` pragma and `isContainerSourceStack(stack)` classifies stacks by tagged frames. Applied at every container evaluation site — `Runtime#loadModule` and `_runModule` (`src/runtime.ts`, covers `require()`, dynamic import and the vite CLI chunk), `src/frameworks/vfs-require.ts`, `next-dev-server.ts`, `next-api-handler.ts`, and the `vm` shim — giving a minification-proof container/host boundary on shared-global threads.
- **Regression tests**: `tests/console-leak.test.ts` drives the full Layer C window — async container output via `globalThis.console` still lands in the stream, while a host-context `console.log` emitted mid-command reaches the original console and never the stream (including a minified-chunk-style stack and the old floating-vue dev-mode stack as predicate cases).
