...
- **ESM→CJS default-import interop** (`src/frameworks/code-transforms.ts`): `import X from 'y'` unwraps `.default` when the module is ESM-shaped, so `@vitejs/plugin-vue` (and other ESM default exports) load correctly. Dynamic imports (`createDynamicImport`, `src/runtime.ts`) probe for `.default` without invoking warn-on-access getters, and modules mixing `export default` with named exports keep `exports.default` (so `__esModule` interop never reads sass-style accessor defaults).
...
- **pnpm `resolutions` / aliases unsupported**: the PackageManager installer (`src/npm/`, `parsePackageSpec`) only handles plain `name@version` specs — it does **not** apply `resolutions`, `overrides`, `npm:` aliases (`"sass-embedded": "npm:sass@1.99.0"`), `workspace:*`, or `link:` specs. A template that declares a `sass-embedded` alias like the official create-vue one will install the *real* native-binary package instead of the pure-JS target, which can't run in a browser container (same reason `resolutions` aliases exist on WebContainers). If this bites again, fix at the platform level: apply `resolutions`/alias resolution in the installer (and handle it in the version resolver), not with a package-specific shim. Workaround today: depend on the pure-JS package directly (e.g. `"sass": "^1.99.0"`).
