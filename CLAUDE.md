...
- Generic runtime/transform/process/shim debug logs are opt-in via `ALMOSTNODE_DEBUG=1` / `globalThis.__ALMOSTNODE_DEBUG__` (`src/shims/diag.ts`); silent by default. The version banner prints by default but can be silenced with `ALMOSTNODE_NO_VERSION=1` / `globalThis.__ALMOSTNODE_NO_VERSION__`.
